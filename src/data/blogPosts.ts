import imgGrinderResidential from "@/assets/gallery/grinder-residential.jpg";
import imgGrinderRootsYard from "@/assets/gallery/grinder-roots-yard.jpg";
import imgGrinderBigStump from "@/assets/gallery/grinder-big-stump.jpg";
import imgGrinderOpenField from "@/assets/gallery/grinder-open-field.jpg";
import imgGrinderAction from "@/assets/gallery/grinder-action.jpg";
import imgGrinderWoods from "@/assets/gallery/grinder-woods.jpg";

export interface BlogPost {
  slug: string;
  title: string;
  excerpt: string;
  image: string;
  date: string;
  readTime: string;
  content: string[];
}

export const blogPosts: BlogPost[] = [
  {
    slug: "why-remove-tree-stumps",
    title: "5 Reasons You Should Remove That Tree Stump",
    excerpt: "That old stump in your yard isn't just ugly — it can attract pests, damage your lawn, and even lower your property value. Here's why you should get it ground down.",
    image: imgGrinderResidential,
    date: "March 10, 2026",
    readTime: "4 min read",
    content: [
      "You finally had that old tree cut down, but now there's an ugly stump sitting in your yard. It might seem harmless, but leaving a stump can cause more problems than you think.",
      "**1. Pest Magnet** — Decaying stumps attract termites, carpenter ants, beetles, and other wood-boring insects. Once they establish a colony in the stump, it's only a matter of time before they move to your home.",
      "**2. Tripping Hazard** — Stumps are a liability, especially if you have kids or elderly family members. Over time, grass grows around them making them even harder to spot.",
      "**3. Property Value** — A well-maintained yard adds curb appeal. A rotting stump does the opposite. If you're thinking about selling, stump removal is one of the easiest ways to boost your home's appearance.",
      "**4. New Growth** — Some stumps sprout new shoots that can be difficult and expensive to manage. Grinding the stump eliminates regrowth entirely.",
      "**5. Lawn Care Hassle** — Mowing around stumps is annoying and can damage your mower. Removing the stump gives you a clean, usable yard.",
      "At Mr. Bones Stump Grinding, we grind stumps below grade so you can replant, resod, or just enjoy a clean yard. Call us at (941) 264-1659 for a free quote.",
    ],
  },
  {
    slug: "stump-grinding-vs-removal",
    title: "Stump Grinding vs. Stump Removal: What's the Difference?",
    excerpt: "Not sure whether to grind or fully remove your stump? We break down the pros, cons, and costs of each method so you can make the best choice.",
    image: imgGrinderAction,
    date: "February 22, 2026",
    readTime: "5 min read",
    content: [
      "When it comes to getting rid of a tree stump, you have two main options: stump grinding and full stump removal. Both get the job done, but they work very differently.",
      "**Stump Grinding** uses a powerful machine to chip the stump into small wood chips, grinding it down 6–12 inches below the soil surface. The root system is left to decompose naturally. This is the most popular and cost-effective method.",
      "**Stump Removal** involves pulling the entire stump and root ball out of the ground using heavy equipment. This leaves a large hole that needs to be filled and is significantly more expensive and disruptive.",
      "**Which Should You Choose?** For most residential and commercial jobs, stump grinding is the way to go. It's faster, cheaper, less invasive, and leaves you with usable mulch. Full removal is typically only necessary for construction projects where the entire root system needs to be cleared.",
      "**Cost Comparison** — Stump grinding typically costs a fraction of full removal. The price depends on the stump's diameter, location, and accessibility. We always provide free, no-obligation estimates.",
      "**The Mr. Bones Advantage** — With three professional-grade stump grinders, we have the right machine for any job. Small backyard stump? No problem. Giant oak stump? We've got you covered. Call (941) 264-1659 today.",
    ],
  },
  {
    slug: "when-to-grind-stumps-florida",
    title: "Best Time to Grind Stumps in Florida",
    excerpt: "Florida's warm climate means you can grind stumps year-round, but there are a few things to consider for the best results.",
    image: imgGrinderBigStump,
    date: "February 8, 2026",
    readTime: "3 min read",
    content: [
      "One of the most common questions we get is: 'When is the best time to grind my stump?' The good news is that in Florida, stump grinding can be done any time of year.",
      "**Year-Round Service** — Unlike northern states where frozen ground can make stump grinding difficult, Florida's mild winters mean the ground stays workable 365 days a year.",
      "**After Tree Removal** — The best time to grind a stump is right after the tree has been cut down. Fresh stumps are easier to grind than old, hardened ones. Plus, you avoid the pest problems that come with leaving a stump to decay.",
      "**Before Hurricane Season** — If you have stumps from previous storms, it's smart to get them ground before June when hurricane season starts. A clean yard means fewer projectiles and hazards if another storm hits.",
      "**Before Landscaping Projects** — Planning to resod, plant new trees, or install a patio? Get your stumps ground first. We grind below grade so the area is ready for whatever comes next.",
      "**Rainy Season Considerations** — Florida's summer rainy season (June–September) can make some yards muddy and harder to access with equipment. If possible, schedule your grinding during the drier months for the smoothest experience.",
      "Ready to schedule? Call Mr. Bones at (941) 264-1659 — we offer same-day service when available!",
    ],
  },
  {
    slug: "how-much-stump-grinding-costs",
    title: "How Much Does Stump Grinding Cost in Sarasota?",
    excerpt: "Get a realistic idea of stump grinding costs in the Sarasota area. We explain what affects pricing and how to get the best value.",
    image: imgGrinderOpenField,
    date: "January 25, 2026",
    readTime: "4 min read",
    content: [
      "One of the first questions homeowners ask is how much stump grinding will cost. The answer depends on several factors, but we'll give you a realistic breakdown for the Sarasota area.",
      "**Size Matters** — The diameter of the stump is the biggest factor in pricing. A small 6-inch stump costs much less than a 36-inch oak stump. We measure at ground level to give you an accurate quote.",
      "**Number of Stumps** — Multiple stumps? We offer volume discounts. The more stumps you need ground, the better the per-stump price gets.",
      "**Accessibility** — If the stump is in an easy-to-reach area, the job goes faster and costs less. Stumps in tight spaces, behind fences, or on slopes may take more time and equipment.",
      "**Root Grinding** — Basic stump grinding removes the stump itself. If you also need surface roots ground down (to protect sidewalks, driveways, or foundations), that's an additional service we offer.",
      "**What's Included** — Our pricing includes the grinding itself plus leaving the wood chip mulch on-site. We can also haul away the mulch or spread it in garden beds for an additional fee.",
      "**Free Estimates** — The best way to know your exact cost is to call us for a free, no-obligation estimate. We'll come to your property, assess the job, and give you a fair, upfront price. Call (941) 264-1659.",
    ],
  },
  {
    slug: "tree-roots-damaging-property",
    title: "Are Tree Roots Damaging Your Property? Here's What to Do",
    excerpt: "Invasive tree roots can crack sidewalks, clog pipes, and lift driveways. Learn how root grinding can save you thousands in repairs.",
    image: imgGrinderRootsYard,
    date: "January 12, 2026",
    readTime: "4 min read",
    content: [
      "Tree roots are powerful — they can lift concrete, crack foundations, invade sewer lines, and destroy irrigation systems. If you're noticing damage, it's time to act.",
      "**Signs of Root Damage** — Cracked or lifted sidewalks, bumpy driveways, slow drains, foundation cracks, and uneven patios are all signs that tree roots are causing problems.",
      "**Surface Root Grinding** — Not all root problems require removing the entire tree. In many cases, we can grind the offending surface roots to stop the damage while preserving the tree's health.",
      "**Preventive Root Barriers** — After grinding, we can recommend root barrier installations to prevent regrowth in sensitive areas like near foundations or plumbing lines.",
      "**When the Tree Needs to Go** — Sometimes the root damage is severe enough that the tree should be removed entirely. In these cases, we can grind the stump and major roots after your tree service takes the tree down.",
      "**Cost of Inaction** — Ignoring root damage only makes it worse. A cracked sidewalk today can become a foundation problem tomorrow. Root grinding is far cheaper than concrete or plumbing repairs.",
      "Don't wait until the damage gets worse. Call Mr. Bones at (941) 264-1659 for a free assessment of your root problems.",
    ],
  },
  {
    slug: "after-hurricane-stump-cleanup",
    title: "Post-Hurricane Stump Cleanup: What Sarasota Homeowners Need to Know",
    excerpt: "After a hurricane, your yard may be full of broken trees and stumps. Here's how to handle cleanup efficiently and safely.",
    image: imgGrinderWoods,
    date: "December 28, 2025",
    readTime: "5 min read",
    content: [
      "Living in Florida means dealing with hurricanes. After a storm passes, many homeowners are left with downed trees, broken stumps, and debris-filled yards. Here's what you need to know about stump cleanup after a hurricane.",
      "**Safety First** — Before doing anything, check for downed power lines, gas leaks, and structural damage. Never attempt to operate equipment near power lines. Call your utility company first.",
      "**Insurance Claims** — Many homeowner's insurance policies cover storm damage cleanup, including stump grinding. Document the damage with photos before any work begins and contact your insurance company early.",
      "**Priority Order** — Focus on stumps that block driveways, threaten structures, or create safety hazards first. Stumps in open areas can wait until access is easier.",
      "**Emergency Response** — At Mr. Bones, we prioritize emergency storm cleanup. We understand the urgency and work to clear hazardous stumps as quickly as possible after a storm.",
      "**Multiple Stump Discounts** — Storms often leave multiple stumps. We offer volume pricing for multi-stump hurricane cleanup jobs, helping you restore your property without breaking the bank.",
      "**Don't Wait** — Leaving storm-damaged stumps to rot invites pests and creates trip hazards. The sooner you get them ground, the sooner your yard recovers. Call (941) 264-1659 for emergency stump grinding.",
    ],
  },
];
